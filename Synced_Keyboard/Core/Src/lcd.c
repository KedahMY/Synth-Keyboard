...
{	
    ...
    // Iterate over x from -LR to +LR
    for (x = -LR; x <= LR; x++) {
        // Solve for y using ellipse equation
        long temp = SR2 - (SR2 * (long)x * (long)x) / LR2;
        if (temp < 0) continue;
        y = (int)(sqrt(temp));
    ...
    // Iterate over y from -SR to +SR
    for (y = -SR; y <= SR; y++) {
        long temp = LR2 - (LR2 * (long)y * (long)y) / SR2;
        if (temp < 0) continue;
        x = (int)(sqrt(temp));
...
//Task 4 Draw the Character.
// Draw a 24x32 bitmap character at position (x,y)
void LCD_DrawBitmap24x32(uint16_t x, uint16_t y, const uint8_t *data, uint16_t color)
{
    uint16_t byteIndex = 0;
    for (uint16_t row = 0; row < 32; row++) {
        for (uint16_t col = 0; col < 24; col++) {
            // Each row has 24 pixels → 3 bytes (24/8 = 3)
            uint8_t byte = data[byteIndex + (col / 8)];
            // Little-endian bit order: LSB is leftmost pixel
            if (byte & (1 << (col % 8))) {
                LCD_DrawDot(x + col, y + row, color);
            }
        }
        byteIndex += 3; // advance to next row (3 bytes per row)
    }
}
